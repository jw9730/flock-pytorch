...
from flock import util
from flock.models import Flock
from run import train_and_validate, test
...
default_finetuning_config = {
    ...
    # ILPC (2)
    "ILPC2022": (1, 1000),  # both small and large
    # Ingram datasets (13)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d",
        "--datasets",
        help="target datasets",
        default="FB15k237Inductive:v1,NELLInductive:v4",
        type=str,
        required=True,
    )
    parser.add_argument(
        "-reps",
        "--repeats",
        help="number of times to repeat each exp",
        default=1,
        type=int,
    )
    parser.add_argument(
        "-ft",
        "--finetune",
        help="finetune the checkpoint on the specified datasets",
        action="store_true",
    )
    parser.add_argument(
        "-tr", "--train", help="train the model from scratch", action="store_true"
    )
    ...
    if args.subdir is not None:
        path = os.path.join(path, args.subdir)
    os.makedirs(path, exist_ok=True)

    results_file = os.path.join(
        path, f"results_{time.strftime('%Y-%m-%d-%H-%M-%S')}.csv"
    )
    ...
    for graph in datasets:
        ...
        for i in range(args.repeats):
            ...
            # TODO: the following is a temporary fix for the largest datasets
            if ds in ["CoDExLarge", "YAGO310", "Hetionet", "DBpedia100k"]:
                print(f"Forcing BPE to {batch_per_epoch} for {ds}")
                cfg.force_bpe = True
            else:
                cfg.force_bpe = False
            ...
            root_dir = os.path.expanduser(
                cfg.output_dir
            )  # resetting the path to avoid inf nesting
            os.chdir(root_dir)
            ...
            if "fast_test" in cfg.train:
                ...
                if util.get_rank() == 0:
                    logger.warning(
                        f"Fast evaluation on {num_val_edges} samples in validation"
                    )
