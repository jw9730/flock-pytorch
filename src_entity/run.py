        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in train_loader:
                ...
                if cfg.force_bpe:
                    if util.get_rank() == 0:
                        print(
                            f"force break at bpe {cfg.train.batch_per_epoch}; current {batch_id_in_epoch}"
                        )
                    if (
                        batch_per_epoch is not None
                        and batch_id_in_epoch >= batch_per_epoch
                    ):
                        break
...
def test(
    ...
    # use `test_batch_size` for testing if it is specified
    if "test_batch_size" in cfg.train:
