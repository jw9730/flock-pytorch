    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        for epoch in range(i, min(cfg.train.num_epoch, i + step)):
            ...
            for batch in tqdm(islice(train_loader, batch_per_epoch)):
                ...
                if (
                    hasattr(cfg.task, "info_nce_temperature")
                    and cfg.task.info_nce_temperature > 0.0
                ):
                    info_nce_target = torch.zeros_like(pred[:, 0], dtype=torch.long)
                    loss += F.cross_entropy(
                        pred / cfg.task.info_nce_temperature, info_nce_target
                    )
                ...
                if util.get_rank() == 0 and batch_id % cfg.train.log_interval == 0:
                    log_losses = losses[-cfg.train.log_interval :]
                    log_loss = (
                    ...
                    logger.warning(separator)
                    logger.warning("binary cross entropy: %g" % log_loss)
