...
class TransductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, device, transform=None, pre_transform=None, **kwargs):
        self.device = device
        super().__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0], weights_only=False)
...
class CoDExSmall(CoDEx):
    ...
    def __init__(self, device, root):
        super().__init__(root=root, device=device, size="s")
...
class CoDExMedium(CoDEx):
    ...
    def __init__(self, device, root):
        super().__init__(root=root, device=device, size="m")
...
class CoDExLarge(CoDEx):
    ...
    def __init__(self, device, root):
        super().__init__(root=root, device=device, size="l")
...
class InductiveDataset(InMemoryDataset):
    ...
    def __init__(
        self, root, version, device, transform=None, pre_transform=None, **kwargs
    ):
...
class ILPC2022SmallInductive(ILPC2022):
    ...
    def __init__(self, root, device, **kwargs):
        super().__init__(root=root, device=device, version="small")


class ILPC2022LargeInductive(ILPC2022):

    def __init__(self, root, device, **kwargs):
        super().__init__(root=root, device=device, version="large")
...
# a joint dataset for pre-training on several graphs
        ...
        self.graphs = [
            (
                self.datasets_map[ds.split(":")[0]](
                    root=root, version=(ds.split(":")[1]), device=device
                )
                if ":" in ds
                else self.datasets_map[ds](root=root, device=device)
            )
            for ds in graphs
        ]
