...
class GrailInductiveDataset(InMemoryDataset):
    ...
    def __init__(
        self, root, version, transform=None, pre_transform=None, merge_valid_test=True
    ):
...
class CoDExSmall(CoDEx):
    ...
    def __init__(self, root):
        super().__init__(root=root, size="s")
...
class CoDExMedium(CoDEx):
    ...
    def __init__(self, root):
        super().__init__(root=root, size="m")
...
class CoDExLarge(CoDEx):
    ...
    def __init__(self, root):
        super().__init__(root=root, size="l")
...
class WikiTopics(InductiveDataset):
    ...
    def processed_dir(self):
        return os.path.join(
            self.root, self.base_name, self.prefix.format(self.version), "processed"
        )
...
class WikiTopicsMeta(WikiTopics):
    ...
    @property
    def raw_file_names(self):
        return ["train.txt", "train.txt", "valid.txt", "valid.txt"]
